...
class LatentSenseClient:
    ...
    def _initiate_upload(self) -> InitiateUploadResponse:
        upload_resp = self._session.post(
            f"{self.base_url}/projects/{self.project_id}/uploads", json={}
        )
    ...
    def _request_run(
        ...
        run_resp = self._session.post(
            f"{self.base_url}{endpoint_path}", json=run_payload
        )
    ...
    async def _get_run_result(self, run_id: str) -> Dict[str, str]:
        while True:
            ...
            status_resp = self._session.get(f"{self.base_url}/runs/{run_id}")
    ...
    async def _wait_for_small_run_result(
        ...
        url = f"{self.base_url}/runs/small/{run_id}/result/{endpoint_suffix}"
        await asyncio.sleep(5)
        while True:
    ...
    async def create_rx_map_small(
        ...
        concepts: Optional list of concepts expected to exist in the corpus or a related rxMap. Particularly useful if you are extending existing rxMap(s), so the new graph can join onto them.
        """
        endpoint = f"{self.base_url}/runs/small/{self.project_id}/rx-map"
        opened_files = []
    ...
    async def get_salient_relationships_small(
        ...
        """
        endpoint = (
            f"{self.base_url}/runs/small/{self.project_id}/relationships-discovery"
        )
        opened_files = []
        try:
            file_parts = {"files": files}
    ...
    async def redact_by_relevance(
        ...
        """
        payload = {"relevance_term": relevance_term, "sensitivity": cutoff}

        return await self._run_multistage(
            endpoint_path="/runs/redact-relevance",
            files=files,
            payload=payload,
    ...
    async def redact_by_relevance_small(
        ...
        sensitivity: A number between 0 and 1 where a lower number results in more redaction.
        """
        endpoint = f"{self.base_url}/runs/small/{self.project_id}/redact-relevance"
        opened_files = []
        try:
            file_parts = {"files": files}
            multipart_payload, opened_files = self._prepare_payload(**file_parts)
    ...
    async def redact_pii_small(
        ...
        files: A list of files to be redacted. Each file can be a path (str)
               or a (filename, content) tuple.
        """
        endpoint = f"{self.base_url}/runs/small/{self.project_id}/redact-pii"
        opened_files = []
        try:
